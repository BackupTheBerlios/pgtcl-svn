 ...
 * Original code:
 *
 *  $RCS: pgtclCmds.c,v 1.67 2006/01/02 05:35:43 schwarzkopf Exp $
 *
 * Identification:
 *
 *  $Id: pgtclCmds.c 25 2006-03-16 20:25:13Z andrew $
 *
 *-------------------------------------------------------------------------
 */
...
translate_escape(char *p, int isArray)
{
    ...
    switch (c = *(p + 1)) {
        ...
        case '"':
            /*
             * Backslashes, curly braces and double-quotes are left escaped
 ...
 *
 *   -error
 *     the error message, if the status indicates error; otherwise
 *     an empty string
 *
 *   -conn
 ...
 *
 *   -cmdTuples
 *     same as -numTuples, but for DELETE and UPDATE
 *
 *   -numAttrs
 ...
 *
 *   -null_value_string null_value
 *     Set the value returned for fields that are null
...
Pg_result(ClientData cData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
    ...
    switch ((enum options) optIndex) {
        ...
        case OPT_ASSIGNBYIDX: {
            ...
            for (tupno = 0; tupno < PQntuples(result); tupno++) {
                CONST84 char *field0 = \
                    PGgetvalue(result, resultid->nullValueString, tupno, 0);
                ...
                for (i = 1; i < PQnfields(result); i++) {
                    ...
                    if (Tcl_ObjSetVar2(interp, arrVarObj, fieldNameObj,
                            Tcl_NewStringObj( PGgetvalue(result, resultid->nullValueString, tupno, i),
                            -1), TCL_LEAVE_ERR_MSG) == NULL) {
                        Tcl_DecrRefCount(fieldNameObj);
                        return TCL_ERROR;
                    }
        ...
        case OPT_LATTRIBUTES: {
            ...
            for (i = 0; i < PQnfields(result); i++) {
                ...
                if (Tcl_ListObjAppendElement(interp, resultObj, subList) == TCL_ERROR)
                    return TCL_ERROR;
            }
            return TCL_OK;
        }
...
Pg_result_errReturn:
    ...
    Tcl_AppendStringsToObj(tresult,
        "\t-status\n",
        "\t-error\n",
        "\t-conn\n",
        "\t-oid\n",
